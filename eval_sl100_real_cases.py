...
from sl100_incident import build_incident_report
from sl100_log_core import local_diagnosis
from sl100_real_cases import LOCAL_CASES_PATH, read_jsonl
from sl100_review_cases import load_candidate_hit
from sl100_es import facts_from_es_search
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="sl100_real_eval_results.json")
...
def evaluate_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    except Exception as exc:  # noqa: BLE001 - real data can expire or become unavailable.
        result["checks"].append({"name": "retrieval", "passed": False, "reason": f"query failed: {exc}"})
        return result

    retrieved = hit is not None
    result["checks"].append({"name": "retrieval", "passed": retrieved, "reason": "matched saved ES id" if retrieved else "saved ES id was not returned"})
    if not hit:
        return result
...
def main() -> int:
    ...
    if not cases:
        print("没有已确认的真实案例。先运行 sl100_review_cases.py collect 和 review。")
        return 2
