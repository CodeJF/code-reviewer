...
from iot_ops_agent.web.models import Incident, NotificationDelivery
...
def enqueue_diagnosis(job_id: str) -> None:
    _enqueue_unique("diagnosis", "iot_ops_agent.web.tasks.execute_diagnosis", job_id, timeout=180)
...
def execute_diagnosis(job_id: str) -> None:
    ...
    try:
        run_diagnosis_job(session_factory, job_id, settings=settings)
    finally:
...
def enqueue_notification(delivery_id: str) -> None:
    ...
    _enqueue_unique(
        "notifications",
        "iot_ops_agent.web.tasks.execute_notification",
        delivery_id,
        timeout=30,
