...
from sl100_incident_types import INCIDENT_TYPE_CATALOG, INCIDENT_TYPE_KEYS, incident_type_label
from sl100_log_core import local_diagnosis, redact_text
from sl100_real_cases import (
...
def main() -> int:
    ...
    if args.command == "collect":
        ...
        for error in [*migration_errors, *errors]:
            print(f"- 跳过数据源: {error}")
        print("下一步: uv run sl100_review_cases.py review")
        return 0
    ...
    print(f"本次完成 {completed} 条评审。下一步: uv run eval_sl100_real_cases.py")
    return 0
