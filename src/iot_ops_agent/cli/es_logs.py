"""
CLI for querying the IoT service Elasticsearch stack through a private gateway.

Examples:
  uv run iot-ops es health
  uv run iot-ops es indices --date 2026-07-09
  uv run iot-ops es search --service gateway --keyword error --date 2026-07-09
  uv run iot-ops es analyze --service deviceShadow --keyword websocket --from "2026-07-09 09:00" --to "2026-07-09 10:00"
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Query IoT Elasticsearch logs through the configured SSH gateway.")
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            ...
            "error": str(exc),
            "next_actions": ["检查私有 SSH 网关、ES 索引映射或时间范围后重试。"],
