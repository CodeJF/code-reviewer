# IoT Ops Agent Elasticsearch 日志接入说明
...
生产环境已有 Elasticsearch + Kibana 日志栈，更适合做真实排障入口。Agent 应先查日志系统，再在必要时回到服务器文件日志做补充验证。仓库中的 `iot-logs` 只是匿名 SSH 别名，实际映射位于 Git 外的私有配置。
...
```text
Host alias: iot-logs
Elasticsearch endpoint: 127.0.0.1:9200 on the private gateway
Authentication: read-only SSH credential
```

本地工具不会直接访问公网 ES，也不会开放端口。查询链路是：

```text
Agent container -> ssh iot-logs -> curl http://127.0.0.1:9200
```
...
优先使用 `iot-ops diagnose`。它接受一句自然语言报障，输出统一 Incident Report。

只看查询计划：

```bash
uv run iot-ops diagnose \
...
```bash
uv run iot-ops es indices --date 2026-07-09
```

按服务、关键词、时间窗口查询：

```bash
uv run iot-ops es search \
...
uv run iot-ops remote search --service deviceShadow --log stderr --keyword panic
uv run iot-ops remote analyze --service pushService --logs error,stderr
```
...
真实场景使用 `iot-ops agent`：

```bash
uv run iot-ops agent "测试说今天上午 9 点多 deviceShadow websocket 异常，帮我查日志"
uv run iot-ops agent "查 gateway 最近一次 error，分析可能原因"
uv run iot-ops agent "查 pushService 最近一小时有没有推送失败"
```
