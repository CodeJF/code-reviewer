...
def replay_snapshot_from_report(candidate: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    """Persist the minimum redacted evidence needed to replay a reviewed case offline."""
    hits = []
    for index, item in enumerate(report.get("evidence", [])[:20], start=1):
        if not isinstance(item, dict) or not item.get("message"):
            continue
        message = redact_text(str(item["message"]))
        leaks = assert_redacted(message)
        if leaks:
            raise RuntimeError(f"replay snapshot failed redaction validation: {', '.join(leaks)}")
        hits.append({
            "id": str(item.get("evidence_id") or f"snapshot-{index}"),
            "message": message,
            "timestamp": str(candidate.get("timestamp", "")),
        })
    if not hits:
        raise ValueError("reviewed report has no safe evidence to snapshot")
    return {
        "schema_version": "1.0",
        "service": str(candidate["service"]),
        "index_pattern": "private-replay",
        "keyword": str(candidate.get("keyword", "")),
        "time_window": report.get("time_window") or {},
        "hits": hits,
        "total": {"value": len(hits), "relation": "eq"},
        "raw_returned": len(hits),
